...
export async function generateMealPlan(weekStarting: string, preferences?: any) {
  ...
  const prompt = `You are a meal planning assistant for a family in Montreal, Quebec. Generate a weekly meal plan starting ${weekStarting}.

Family Details:
- Adults: ${prefs.adults.join(', ')}
- Children: ${prefs.children.map((c: any) => `${c.name} (age ${c.age}) - favorites: ${c.favorites.join(', ')}`).join('; ')}
- Office days (need quick meals): ${prefs.office_days.join(', ')}
- Budget: $${prefs.budget.min}-$${prefs.budget.max} CAD per week
- Location: ${prefs.location}

Requirements:
1. Create 7 dinner meals (one for each day)
2. For office days (${prefs.office_days.join(', ')}), suggest meals that take 30 minutes or less
...
5. Consider the child's favorites: ${prefs.children[0].favorites.join(', ')}
